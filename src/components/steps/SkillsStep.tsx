...
export function SkillsStep({
  ...
  const [isRequestingSkill, setIsRequestingSkill] = useState(false);
  ...
  const addSkill = () => {
    ...
    if (newSkill && skillLevel && skillExists) {
      ...
      if (milestoneMessages[skillCount]) {
        const randomMilestone = milestoneMessages[skillCount][Math.floor(Math.random() * milestoneMessages[skillCount].length)];
      ...
      addMotivationalMessage("skill", randomMessage, `${newSkill}${versionText} ahora forma parte de tu stack técnico`, "🔧", `Nivel: ${levelTxt}`);
  ...
  const addSkillFromPanel = () => {
    ...
    const randomMessage = skillMessages[Math.floor(Math.random() * skillMessages.length)];
    const versionText = tempSkillVersion.trim() ? ` ${tempSkillVersion.trim()}` : "";
  ...
  const removeSkill = (index: number) => {
    ...
    updateFormData("skills", skills.filter((_, i) => i !== index));
    ...
    <div className="space-y-8">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {skillInfo && (
                        <Badge className={`text-xs ${skillInfo.color} border-0`}>
                          {skillInfo.category}
                        </Badge>
                      )}
      ...
      <div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h4 className="text-lg font-bold text-[var(--axity-purple)]">
              Tecnologías populares en Axity 🔥
            </h4>
        ...
        {searchTerm && (
          ...
          >
            ...
            <span>
              {filteredSkills.length} tecnología{filteredSkills.length !== 1 ? "s" : ""} encontrada{filteredSkills.length !== 1 ? "s" : ""}
              {filteredSkills.length > 0 && (
                <span className="text-[var(--axity-purple)] font-medium ml-1">
                  para "{searchTerm}"
                </span>
              )}
            </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {filteredSkills.map((skill, index) => {
            ...
            const isAlreadyAdded = (formData.skills || []).some((s) => s.name === skill);
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="space-y-1">
                      <h5 className={`font-medium text-sm ${isAlreadyAdded ? "text-gray-500" : "text-gray-800"}`}>
                        {skill}
                      </h5>
                      {skillInfo && (
                    ...
                    {isAlreadyAdded && (
                      <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute -top-1 -right-1 bg-[var(--axity-mint)] text-white rounded-full p-1"
                      >
      ...
      <div className="mt-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h4 className="text-lg font-bold text-[var(--axity-purple)]">
              ¿No encuentras una tecnología? 🔍
            </h4>
            <p className="text-sm text-gray-500">
              Agrega cualquier tecnología manualmente
            </p>
          </div>
        </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm text-amber-800 mb-3">
                            ...
                            {hasSkillLevel ? (
                              "¿Te gustaría solicitarla para agregarla?"
                            ) : (
                              "Selecciona un nivel de experiencia y podrás solicitarla."
                            )}
                          </p>
        ...
        <SheetContent side="right" className="w-[calc(100vw-1.5rem)] max-w-none sm:w-[800px] lg:w-[950px] xl:w-[1100px] 2xl:w-[1200px] overflow-y-auto sm:mr-0 mr-3">
              ...
              <div className="relative flex items-center gap-6 sm:gap-8">
                ...
                <div className="flex flex-col items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                    tempSkillLevel 
                      ? 'bg-gradient-to-br from-[var(--axity-mint)] to-emerald-500' 
                      : 'bg-gray-200'
                  }`}>
                    {tempSkillLevel ? (
                  ...
                  <span className={`text-xs font-medium ${
                    tempSkillLevel ? 'text-[var(--axity-mint)]' : 'text-gray-400'
                  }`}>
                    Nivel
                  </span>
                </div>
                ...
                {skillLevels.map((level) => (
                  <motion.div
                    ...
                    className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      tempSkillLevel === level.id
                        ? "border-[var(--axity-violet)] bg-[var(--axity-violet)]/10"
                        : "border-gray-200 hover:border-[var(--axity-violet)]/50"
                    }`}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-[var(--axity-purple)]">
                          {level.name}
                        </h4>
                        <p className="text-sm text-gray-600">
                          {level.description}
                      ...
                      {tempSkillLevel === level.id && (
                        <CheckCircle2 className="h-5 w-5 text-[var(--axity-violet)]" />
                      )}
                    </div>
                  </motion.div>
                ))}
          ...
          <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 lg:p-6 mt-8">
            <div className="flex gap-3">
              ...
              <Button
                onClick={addSkillFromPanel}
                disabled={!tempSkillLevel}
            ...
            {!tempSkillLevel && (
              <p className="text-xs text-gray-500 text-center mt-2">
                Selecciona un nivel de experiencia para continuar
              </p>
            )}
          </div>
        </SheetContent>
